...
parser$add_argument("--list", dest="LIST", help="只展示文件内通路，每个一行", default = NULL)
parser$add_argument("--show_number", dest="NUMBER", 
                    help="展示的通路数目，如果指定了 --list 参数，那么此参数无效。默认 25", 
                    default = 25)
parser$add_argument("--row_name_width", dest="ROWWIDTH", help = "通路名字符宽度，超过此宽度将换行显示，默认 40", 
                    default = 40)
...
parser$add_argument("--p_min_color", dest = "P_MIN_COLOR", help = "图例 P 值刻度（小）颜色。默认：#890011", 
                    default = "#890011")
parser$add_argument("--p_max_color", dest = "P_MAX_COLOR", help = "图例 P 值刻度（大）颜色。默认：#f87c5e", 
                    default = "#f87c5e")
